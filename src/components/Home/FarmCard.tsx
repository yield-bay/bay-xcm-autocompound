...
import toDollarUnits, { formatTokenSymbols, replaceTokenSymbols } from '@utils/farmMethods';
...
const FarmCard: FC<Props> = ({ farm }) => {
  const tokenNames = formatTokenSymbols(
    replaceTokenSymbols(farm?.asset.symbol)
  );
  return (
    <div
      className="flex flex-row group w-full max-w-[1138px] bg-baseGrayDark hover:ring-[1px] transition duration-20 ring-primaryGreen rounded-lg"
    >
      {/* LEFT */}
      <div className="flex flex-row items-center justify-between px-6 py-14 w-full max-w-[400px] rounded-l-lg bg-card-gradient">
        ...
        <div className="flex flex-col gap-y-[10px]">
          <div className="flex flex-row gap-x-3">
            ...
            <button className="py-[9px] px-4 bg-white text-[10px] leading-[13.5px] text-black rounded-full hover:bg-offWhite transition-all duration-150">
              Standard Swap
            </button>
          </div>
        ...
        <button className="rounded-full scale-0 group-hover:scale-100 bg-white p-[10px] h-fit hover:bg-offWhite text-black transition-all duration-150">
          <Image
            height={28}
            width={28}
