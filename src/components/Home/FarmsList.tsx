...
const FarmsList: FC<Props> = ({ farms }) => {
  return (
    <div className="flex flex-col items-center gap-y-[25px] py-24">
      {farms.map((farm, index) => {
        const tokenNames = formatTokenSymbols(
          replaceTokenSymbols(farm?.asset.symbol)
        );
        return (
          <div className="flex flex-row group w-full max-w-[1138px] bg-baseGrayDark hover:ring-[1px] transition duration-20 ring-primaryGreen rounded-lg">
            {/* LEFT */}
