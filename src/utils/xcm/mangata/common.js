...
class AutoCompound {
  ...
  run = async () => {
    ...
    const mgxToken = account.getAssetByChainAndSymbol(
      mangataChainName,
      mangataNativeToken.symbol
    );
    const turToken = account.getAssetByChainAndSymbol(
      mangataChainName,
    ...
    const shouldMintLiquidity = await confirm({
      message: `\nAccount balance check is completed and proxy is set up. Press ENTRE to mint ${poolName}.`,
      default: true,
    });
    ...
    if (shouldMintLiquidity) {
      ...
      const answerPool = await confirm({
        message:
          '\nDo you want to continue to schedule auto-compound. Press ENTRE to continue.',
        default: true,
      });
      ...
      if (answerPool) {
        ...
        const twoHoursExecutionTimeout = calculateTimeout(
        ...
        console.log(
          `\n6. Keep Listening events on ${mangataChainName} until ${moment(
            ...
            'YYYY-MM-DD HH:mm:ss'
          )}(${timestampTwoHoursLater}) to verify that the task was successfully canceled ...`
        );
