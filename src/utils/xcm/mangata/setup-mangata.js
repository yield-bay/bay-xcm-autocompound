...
async function main() {
  ...
  const answerPool = await confirm({
    message: '\nAccount setup is completed. Press ENTRE to set up pools.',
    default: true,
  });
  ...
  if (answerPool) {
    ...
    const poolFound = _.find(
      pools,
      (pool) =>
        pool.firstTokenId ===
          mangataHelper.getTokenIdBySymbol(mangataNativeToken.symbol) &&
        pool.secondTokenId ===
          mangataHelper.getTokenIdBySymbol(turingNativeToken.symbol)
    );
    ...
    await mangataHelper.updatePoolPromotion(
